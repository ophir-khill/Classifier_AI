...
class ID3:
    ...
    def find_best_split(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        for field_num in range(rows.shape[1]):  # Go over all fields
            field_values = np.sort(rows[:, field_num])
            for i in range(len(field_values) - 1):  # Go over all relevant threshold
                new_question = Question(self.label_names[field_num + 1], field_num, (field_values[i] + field_values[i+1])/2)
